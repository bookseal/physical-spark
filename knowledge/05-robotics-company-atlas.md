...
Physical AI 산업을 "회사 이름 60개"로 외우면 금방 뒤죽박죽이 된다. 대신 **나무 한 그루**로 보면 각 회사가 "어느 층·어느 잎"인지로 정리된다. 뿌리에서 잎까지 5개 층이고, 아래로 갈수록 상품화(값 떨어짐), 위로 갈수록 자본이 몰린다.

```mermaid
flowchart TB
  APP["⑤ <b>응용 산업 (잎)</b><br/>우주·국방·물류·제조·휴머노이드·건설"]
  FDE["④ <b>배포·통합 · FDE</b><br/>안전 · 모션플래닝 · 현장 로봇브레인 · 배포 플랫폼<br/><i>진짜 병목 — 사람이 부족</i>"]
  BRAIN["③ <b>파운데이션 모델 (뇌)</b><br/>VLA · π0 · Skild Brain<br/><i>자본이 가장 집중</i>"]
  EYE["② <b>센서·인지 (눈)</b><br/>카메라 · 인지 모델<br/><i>조작의 남은 난제</i>"]
  BODY["① <b>액추에이터·기체 (몸)</b><br/>팔 · 손 · 모터 · 휴머노이드 바디<br/><i>상품화 — 마진 압박</i>"]
  BODY --> EYE --> BRAIN --> FDE --> APP
  classDef body fill:#2ec4ff,stroke:#1289c9,color:#04120f
  classDef eye fill:#35e08a,stroke:#1b7a4a,color:#04120f
  classDef brain fill:#39e0c8,stroke:#0bb3a0,color:#04120f
  classDef fde fill:#ffcf4d,stroke:#c98a00,color:#04120f
  classDef app fill:#ff5d8f,stroke:#c22a5a,color:#fff
  class BODY body
  class EYE eye
  class BRAIN brain
  class FDE fde
...
## 8.5. 돈의 타임라인 — 2025~2026 메가라운드

숫자로 보면 흐름이 잡힌다. **2025년은 응용·물류가, 2026년 초는 뇌(파운데이션 모델)와 휴머노이드가** 초대형 수표를 받았다. 밸류에이션이 매출이 아니라 **기대**임을 잊지 말 것 — 특히 휴머노이드·뇌는 아직 증명 단계다.

```mermaid
timeline
  title 로봇 메가라운드 (조달액 @ 밸류에이션)
  2025 H1 : Dexterity $95M @ $1.65B (물류) : GrayMatter $45M (제조)
  2025 H2 : Figure $1B @ $39B (휴머노이드) : Field AI $405M @ $2B (뇌) : Hadrian $260M (제조) : Forterra $238M @ $1B (국방)
  2026 H1 : Skild $1.4B @ $14B (뇌) : Neura $1.4B @ $7B (휴머노이드) : Apptronik $935M @ $5B (휴머노이드) : Agility SPAC $2.5B (휴머노이드) : Scout AI $100M (국방 VLA)
```

> **읽는 법:** 가장 큰 밸류에이션(Figure $39B, Skild $14B)은 전부 **뇌 또는 휴머노이드**다 — 매출이 아니라 "여러 몸에 이식되는 범용성"에 대한 베팅. 반면 실제로 돈을 벌며 배치되는 물류·제조는 수표 크기가 상대적으로 작다. 이 괴리가 §0의 "몸은 상품화, 뇌에 자본 집중" 그림 그대로다.
...
피지컬 스파크의 각: **이 글로벌 로봇들을 한국 현장에 들여왔을 때 SW로 세팅하고 최적화하는 '디플로이먼트 엔지니어(FDE)'를 벼려내는 프루빙 그라운드.** 위 기업들이 곧 우리 참가자의 미래 취업·파트너십 시장이다.
...
## 관련 (Related)
- 🎨 [[atlas-dashboard|학습 대시보드 (인터랙티브 HTML)]] — 이 문서의 컬러 버전: 필터·다이어그램·타임라인
