...
The **data + talent engine** for **Physical AI** — *Kaggle for robot arms*. Developers solve gamified robot missions, peer-review each other's runs on video, and climb a board.

> 🌐 **Live:** **https://physical-spark.bit-habit.com** — playable landing, missions, and the join page.
...
We give a player a robot-arm mission. They build it, they solve it, and
they upload a short video of the result. Then players review each other's
runs. The ones that survive review rise to the top of the board.

Two things stack up while they play: a **verified robot demonstration**
(LeRobot format, with the peer review attached as the quality label) and a
**proof-of-work record** of what that person can actually do. We sell both
to robotics companies. The missions are how they arrive.

![Physical Spark — landing page](assets/screenshot-landing.png)

The idea mixes two things:

- The **peer-review** model of École 42 — people level up by reviewing
  each other's work, not by listening to a lecturer.
...
This is not a textbook. It is a **data + talent engine**.
...
- [**Curriculum vision — start and end**](docs/01-concept/curriculum-vision.md) — the ladder from a \$0 MacBook sim to the final mission ("say *pick that up* and it does"), and — more importantly — [what we deliberately cut](docs/01-concept/curriculum-vision.md#3-무엇을-빼는가-이게-더-중요하다): ROS, embedded, LiDAR, five-finger hands.
- [Market landscape](docs/02-market/market-landscape.md) — US & China scan: HF/LeRobot, NVIDIA, Makeblock, DJI, and the gaps.
- [**Global market — who pays**](docs/02-market/global-market.md) — Korea / US / Singapore run on three different engines (state budget, enterprise L&D, state credit via an institution), with sourced 2026 numbers — and the place the standard advice is **wrong**: a six-week online cohort is *not* SkillsFuture-eligible in Singapore. Live page: [market.html](https://physical-spark.bit-habit.com/market.html).
...
- [Teardown — HF LeRobot vs NVIDIA](lab/references/course-analysis-hf-nvidia.md) — the two flagship tracks, dissected as raw material for our own mission map.
...
- **[A robot's anatomy — arm, hand, eye](knowledge/04-arm-hand-eye.md)** — cutting the robot by *part*
  ...
  are all RGB-only. That claim is about the robot's *legs*. Our missions live in its *hands*.
...
- 🗺️ **Next:** peer-review upload + gallery; more missions; Apple passkey login.
